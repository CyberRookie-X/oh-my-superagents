...
`oh-my-superagents` 正在演进为一个更通用的路由与 OMS 控制平面产品，当前在 OpenCode、Codex、Qwen、Claude Code 和 Copilot CLI 上提供一等公民级别的 `superpowers` 支持，并包含一个覆盖 OpenCode、Codex、Qwen 的实验性、宿主原生 direct mode。
...
| 宿主引导/Bootstrap | 原生插件入口 | 本地 bootstrap / plugin bundle | 暂无 | 暂无 | 插件 + 钩子 |
| 上游兼容性监控 | 完整支持 | 完整支持 | 暂未实现 | 暂未实现 | 完整支持 |
| 生成宿主工件 | Agents + Commands | Agents + Plugin/Skills | Agents + Commands | Skills | Agents + Skills + Plugin + Hooks |
| 临时停用 helper | 完整支持 | 完整支持 | 暂未实现 | 暂未实现 | 完整支持 |
| `codexFast` | 完整支持 | 完整支持 | 暂未实现 | 暂未实现 | N/A |
...
- Copilot CLI 支持使用 Agent + Plugin + Hooks 方案实现完整的宿主深度支持，生成 `.github/copilot/` 目录下的代理、技能、插件和钩子文件。
...
| Copilot 适配层 | `src/copilot.ts` | 283 | 薄 |
...
当前它仍然在已支持宿主上提供一等公民级别的 `superpowers` 支持，而首个通用切片现在已经扩展为覆盖 OpenCode、Codex、Qwen 的实验性宿主原生 direct mode；Claude Code 当前只支持 `superpowers` 这条 slice。Copilot CLI 支持现已可用，提供完整的 Agent + Plugin + Hooks 集成。
