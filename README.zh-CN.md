...
`oh-my-superagents` 正在演进为一个更通用的路由与 OMS 控制平面产品，当前在 OpenCode、Codex、Qwen、Claude Code、GitHub Copilot CLI 上提供一等公民级别的 `superpowers` 支持，并包含一个覆盖 OpenCode、Codex、Qwen 的实验性、宿主原生 direct mode。
...
| 宿主引导/Bootstrap | 原生插件入口 | 本地 bootstrap / plugin bundle | 暂无 | 暂无 | Plugin marketplace |
| 上游兼容性监控 | 完整支持 | 完整支持 | 暂未实现 | 暂未实现 | 计划中 |
| 生成宿主工件 | Agents + Commands | Agents + Plugin/Skills | Agents + Commands | Skills | Agents + Skills + Hooks |
| 临时停用 helper | 完整支持 | 完整支持 | 暂未实现 | 暂未实现 | 通过 Hooks |
...
| Copilot 适配 + hooks | `src/copilot.ts`、`src/copilot-hooks.ts`、`src/copilot-bootstrap.ts` | 350 | 薄 |
| 上游兼容性监控 | `src/superpowers-compatibility.ts`、`src/superpowers-detectors.ts` | 1093 | 中等 |
| 共享工件协调层 | `src/materialize.ts` | 729 | 中薄 |
